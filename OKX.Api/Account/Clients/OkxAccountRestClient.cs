...
public class OkxAccountRestClient(OkxRestApiClient root) : OkxBaseRestClient(root)
{
    ...
    public Task<RestCallResult<List<OkxAccountPositionBuilder>>> PositionBuilderAsync(
        ...
        decimal? leverage = null,
        IEnumerable<OkxAccountSimulatedPositionRequest>? simulatedPositions = null,
        IEnumerable<OkxAccountSimulatedAssetRequest>? simulatedAssets = null,
    ...
    public Task<RestCallResult<List<OkxAccountPositionBuilderGraph>>> PositionBuilderAsync(
        ...
        IEnumerable<OkxAccountSimulatedPositionRequest>? simulatedPositions = null,
        IEnumerable<OkxAccountSimulatedAssetRequest>? simulatedAssets = null,
        OkxAccountSimulatedMmrRequest? simulatedMmr = null,
        CancellationToken ct = default)
    {
        var parameters = new ParameterCollection();
        parameters.AddOptional("inclRealPosAndEq", importExisting);
    ...
    public Task<RestCallResult<OkxAccountMovePosition>> MovePositionsAsync(OkxAccountMovePositionRequest request, CancellationToken ct = default)
    {
        ...
        return ProcessOneRequestAsync<OkxAccountMovePosition>(GetUri("api/v5/account/move-positions"), HttpMethod.Post, ct, signed: true, bodyParameters: parameters);
    }
    ...
    /// </summary>
    /// <param name="blockTradeId">BlockTdId generated by the system</param>
    ...
    /// <param name="state">Positions transfer state, filled pending</param>
    /// <param name="begin">Filter with a begin timestamp. Unix timestamp format in milliseconds (inclusive)</param>
    /// <param name="end">Filter with an end timestamp. Unix timestamp format in milliseconds (inclusive)</param>
    /// <param name="limit">Number of results per request. The maximum and default are both 100</param>
    /// <param name="ct">Cancellation Token</param>
    /// <returns></returns>
    public Task<RestCallResult<List<OkxAccountMovePosition>>> GetMovePositionsHistoryAsync(
        long? blockTradeId = null,
        string? clientOrderId = null,
        OkxAccountMovePositionState? state = null,
        long? begin = null,
        long? end = null,
        int limit = 100,
        CancellationToken ct = default)
    {
        ...
        parameters.AddOptional("beginTs", begin);
        parameters.AddOptional("endTs", end);
        parameters.AddOptional("limit", limit);

        return ProcessListRequestAsync<OkxAccountMovePosition>(GetUri("api/v5/account/move-positions-history"), HttpMethod.Get, ct, signed: true, queryParameters: parameters);
    }
    ...
    /// </summary>
    /// <param name="currency">Currency</param>
    /// <param name="action">Auto earn operation action</param>
    /// <param name="type">Auto earn type</param>
    /// <param name="ct">Cancellation Token</param>
    /// <returns></returns>
    public Task<RestCallResult<OkxAccountAutoEarn>> SetAutoEarnAsync(string currency, OkxAccountAutoEarnAction action, OkxAccountAutoEarnType? type = null, CancellationToken ct = default)
    {
        var parameters = new ParameterCollection
        {
            { "ccy", currency },
        };
        parameters.AddEnum("action", action);
        parameters.AddOptionalEnum("earnType", type);

        return ProcessOneRequestAsync<OkxAccountAutoEarn>(GetUri("api/v5/account/set-auto-earn"), HttpMethod.Post, ct, signed: true, bodyParameters: parameters);
    }

    /// <summary>
    /// Only applicable to USD-margined contract.
    /// </summary>
    /// <param name="currency">USD-margined contract settle currency</param>
    /// <param name="ct">Cancellation Token</param>
    /// <returns></returns>
    public Task<RestCallResult<OkxAccountSettleCurrency>> SetSettleCurrencyAsync(string currency, CancellationToken ct = default)
    {
        var parameters = new ParameterCollection
        {
            { "settleCcy", currency },
